...
export function InstallerLayout({
  ...
  return (
    <div className={`${desktop ? 'h-full w-full flex items-center justify-center' : 'bg-zinc-50 text-zinc-900 flex items-center justify-center p-4'} font-sans selection:bg-blue-100 selection:text-blue-900`}>
      {/* Main Window */}
      <div className={`w-full ${desktop ? 'h-full max-w-full' : 'max-w-5xl h-[900px]'} bg-white ${desktop ? '' : 'rounded-2xl shadow-2xl'} border border-zinc-200 flex overflow-hidden relative`}>
        {/* Sidebar */}
        <div className="w-64 bg-zinc-50/50 border-r border-zinc-200 flex flex-col pt-8 pb-4 relative z-10 backdrop-blur-xl">
          {/* Window Controls (Decorative) - Hidden in desktop mode */}
          {!desktop && (
