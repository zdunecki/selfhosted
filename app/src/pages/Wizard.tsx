...
import { apiFetch, getApiBaseUrl, getAssetUrl } from '../utils/api'
...
import type { WizardState, WizardActions, GCPBillingAccount, GCPProject } from './wizard/types'
...
export function Wizard() {
    const { apps, providers, loading, error } = useWizardData()
    
    // Ensure apps and providers are always arrays (defensive check)
    ...
    // Derived State
    const selectedApp = safeApps.find(a => a.name === appName)
    const selectedProvider = safeProviders.find(p => p.name === providerName)
    ...
    // Helper function to get app logo (with backend URL prefix for desktop mode)
    ...
    // Helper function to get provider logo (with backend URL prefix for desktop mode)
    ...
    useEffect(() => {
        if (providerName) {
            ...
            apiFetch<{ hasCredentials?: boolean }>(`/api/providers/check?provider=${providerName}`)
                .then(data => {
            ...
            apiFetch<Region[]>(`/api/regions?provider=${providerName}`)
                .then(data => {
                    setRegions(data || [])
                })
                .catch(err => {
    ...
    useEffect(() => {
        if (providerName && region && !showConfig) {
            ...
            apiFetch<Size[]>(`/api/sizes?provider=${providerName}&region=${encodeURIComponent(region)}`)
    ...
    useEffect(() => {
        const checkDomain = async () => {
            ...
            try {
                const data = await apiFetch<{ provider?: string }>(`/api/domains/check?domain=${domainAuto}`)
    ...
    useEffect(() => {
        ...
        const runBilling = async () => {
            ...
            setGcpBillingAccountsError(null)
            try {
                const accounts = await apiFetch<GCPBillingAccount[]>('/api/providers/gcp/billing-accounts')
                if (cancelled) return
                ...
                if (gcpProjectMode === 'new' && !gcpBillingAccount) {
                    if (open.length === 1) {
                        ...
                        void apiFetch('/api/providers/config', {
                            method: 'POST',
                            body: JSON.stringify({
        ...
        const runProjects = async () => {
            ...
            setGcpProjectsError(null)
            try {
                const projects = await apiFetch<GCPProject[]>('/api/providers/gcp/projects')
                if (cancelled) return
    ...
    const handleSaveToken = async () => {
        try {
            ...
            await apiFetch('/api/providers/config', {
                method: 'POST',
                body: JSON.stringify({
                    provider: providerName,
    ...
    const handleVerifyCloudflareToken = async () => {
        ...
        try {
            ...
            try {
                const data = await apiFetch<{ success?: boolean; errors?: Array<{ message: string }>; message?: string }>('/api/cloudflare/verify', {
                    method: 'POST',
                    body: JSON.stringify({
                ...
                if (data.errors && data.errors.length > 0) {
                    const errorMessage = data.errors[0].message || data.message || 'Failed to verify token'
                ...
                const errorMessage = err.message || 'Failed to verify token'
                setCloudflareTokenError(errorMessage)
                setCloudflareTokenVerified(false)
            }
        } catch (err) {
    ...
    const sendPTY = async (sessionId: string, data: string) => {
        ...
        const baseUrl = await getApiBaseUrl()
        await fetch(`${baseUrl}/api/pty/input`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
