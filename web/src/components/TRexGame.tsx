import { useEffect, useRef, useState } from 'react'
import '../t-rex/index.css'

// Import the game script - we'll need to make it work with React
// The game script expects to be loaded globally
declare global {
    interface Window {
        Runner: any
    }
}

export function TRexGame() {
    const containerRef = useRef<HTMLDivElement>(null)
    const [showMessage, setShowMessage] = useState(true)
    const runnerInstanceRef = useRef<any>(null)

    useEffect(() => {
        if (!containerRef.current) return

        // Load the script dynamically
        const loadScript = () => {
            return new Promise<void>((resolve, reject) => {
                // Check if already loaded
                if (window.Runner) {
                    resolve()
                    return
                }

                // Check if script is already in DOM
                const existingScript = document.querySelector('script[data-trex-game]')
                if (existingScript) {
                    // Wait for Runner to be available
                    const checkRunner = setInterval(() => {
                        if (window.Runner) {
                            clearInterval(checkRunner)
                            resolve()
                        }
                    }, 50)
                    setTimeout(() => {
                        clearInterval(checkRunner)
                        if (!window.Runner) {
                            reject(new Error('Runner not available'))
                        }
                    }, 5000)
                    return
                }

                const script = document.createElement('script')
                script.src = '/t-rex-game.js'
                script.async = true
                script.setAttribute('data-trex-game', 'true')
                
                script.onload = () => {
                    // Wait a bit for Runner to be defined
                    setTimeout(() => {
                        if (window.Runner) {
                            resolve()
                        } else {
                            reject(new Error('Runner class not found'))
                        }
                    }, 100)
                }
                
                script.onerror = () => {
                    reject(new Error('Failed to load T-Rex game script'))
                }
                
                document.head.appendChild(script)
            })
        }

        const initGame = async () => {
            try {
                await loadScript()

                if (!containerRef.current || !window.Runner) {
                    return
                }

                // Wait for container to be ready and images to load
                const waitForImages = () => {
                    return new Promise<void>((resolve) => {
                        const img1x = document.getElementById('offline-resources-1x') as HTMLImageElement
                        const img2x = document.getElementById('offline-resources-2x') as HTMLImageElement
                        
                        if (!img1x || !img2x) {
                            setTimeout(waitForImages, 100)
                            return
                        }

                        const checkComplete = () => {
                            if (img1x.complete && img2x.complete && img1x.naturalWidth > 0 && img2x.naturalWidth > 0) {
                                resolve()
                            } else {
                                setTimeout(checkComplete, 100)
                            }
                        }

                        if (img1x.complete && img2x.complete && img1x.naturalWidth > 0 && img2x.naturalWidth > 0) {
                            resolve()
                        } else {
                            img1x.onload = () => checkComplete()
                            img2x.onload = () => checkComplete()
                            img1x.onerror = () => {
                                console.warn('1x sprite failed to load, continuing anyway')
                                resolve()
                            }
                            img2x.onerror = () => {
                                console.warn('2x sprite failed to load, continuing anyway')
                                resolve()
                            }
                            // Timeout after 5 seconds
                            setTimeout(() => {
                                console.warn('Image load timeout, continuing anyway')
                                resolve()
                            }, 5000)
                        }
                    })
                }

                setTimeout(async () => {
                    if (!containerRef.current || !window.Runner) return

                    // Wait for images to load
                    await waitForImages()

                    // Patch setArcadeModeContainerScale to prevent scaling
                    if (window.Runner && window.Runner.prototype && !window.Runner.prototype._patchedSetArcadeModeContainerScale) {
                        window.Runner.prototype.setArcadeModeContainerScale = function(this: any) {
                            // Don't apply scaling - just return without doing anything
                            return
                        }
                        window.Runner.prototype._patchedSetArcadeModeContainerScale = true
                    }

                    // Patch the game's loadSounds to work with DocumentFragment
                    // getElementById doesn't work on DocumentFragment, so we patch it to use querySelector
                    window.Runner.prototype.loadSounds = function(this: any) {
                        // @ts-ignore - IS_IOS is defined in the game script
                        if (typeof IS_IOS !== 'undefined' && IS_IOS) {
                            return // Skip on iOS
                        }
                        
                        this.audioContext = new AudioContext();

                        const templateEl = document.getElementById(this.config.RESOURCE_TEMPLATE_ID) as HTMLTemplateElement
                        if (!templateEl || !templateEl.content) {
                            console.warn('Audio template not found')
                            return
                        }

                        const resourceTemplate = templateEl.content

                        // Patch getElementById on DocumentFragment to use querySelector
                        if (!resourceTemplate.getElementById || typeof resourceTemplate.getElementById !== 'function') {
                            // @ts-ignore - Adding getElementById to DocumentFragment
                            resourceTemplate.getElementById = function(this: DocumentFragment, id: string) {
                                return this.querySelector('#' + id) as HTMLElement | null
                            }
                        }

                        // @ts-ignore - Runner.sounds is defined in the game script
                        for (var sound in Runner.sounds) {
                            // @ts-ignore
                            var soundEl = resourceTemplate.getElementById(Runner.sounds[sound])
                            if (!soundEl) {
                                // @ts-ignore
                                console.warn(`Audio element ${Runner.sounds[sound]} not found`)
                                continue
                            }
                            var soundSrc = (soundEl as HTMLAudioElement).src;
                            soundSrc = soundSrc.substr(soundSrc.indexOf(',') + 1);
                            // @ts-ignore - decodeBase64ToArrayBuffer is defined in the game script
                            var buffer = decodeBase64ToArrayBuffer(soundSrc);

                            // Async, so no guarantee of order in array.
                            this.audioContext.decodeAudioData(buffer, function (this: any, index: string, audioData: AudioBuffer) {
                                this.soundFx[index] = audioData;
                            }.bind(this, sound));
                        }
                    }

                    // Reset singleton if exists
                    if (window.Runner.instance_) {
                        const prevInstance = window.Runner.instance_
                        try {
                            if (prevInstance.stop) prevInstance.stop()
                            if (prevInstance.containerEl) {
                                prevInstance.containerEl.innerHTML = ''
                            }
                        } catch (e) {
                            console.warn('Error cleaning previous instance:', e)
                        }
                        window.Runner.instance_ = null
                    }
                    
                    // Initialize the game
                    runnerInstanceRef.current = new window.Runner('.interstitial-wrapper')
                    
                    // Clear any transform that might have been applied
                    setTimeout(() => {
                        const runnerContainer = document.querySelector('.runner-container') as HTMLElement
                        if (runnerContainer) {
                            runnerContainer.style.transform = 'none'
                        }
                    }, 100)
                }, 300)
            } catch (error) {
                console.error('Failed to initialize T-Rex game:', error)
            }
        }

        initGame()

        // Cleanup
        return () => {
            if (runnerInstanceRef.current) {
                try {
                    if (runnerInstanceRef.current.stop) {
                        runnerInstanceRef.current.stop()
                    }
                } catch (e) {
                    // Ignore cleanup errors
                }
            }
        }
    }, [])

    const handleKeyDown = (e: React.KeyboardEvent) => {
        if (e.keyCode === 32 || e.key === ' ') {
            setShowMessage(false)
        }
    }

    useEffect(() => {
        const handleGlobalKeyDown = (e: KeyboardEvent) => {
            if (e.keyCode === 32 || e.key === ' ') {
                setShowMessage(false)
            }
        }

        window.addEventListener('keydown', handleGlobalKeyDown)
        return () => {
            window.removeEventListener('keydown', handleGlobalKeyDown)
        }
    }, [])

    return (
        <div 
            ref={containerRef}
            id="t" 
            className="offline"
            onKeyDown={handleKeyDown}
            tabIndex={0}
            style={{ width: '100%', height: '100%', position: 'relative' }}
        >
            {showMessage && (
                <div 
                    id="messageBox" 
                    className="sendmessage"
                    style={{ 
                        visibility: showMessage ? 'visible' : 'hidden',
                        position: 'absolute',
                        top: '50%',
                        left: '50%',
                        transform: 'translate(-50%, -50%)',
                        zIndex: 1000,
                        textAlign: 'center',
                        fontFamily: "'Open Sans', sans-serif"
                    }}
                >
                    <h1 style={{ textAlign: 'center', fontFamily: "'Open Sans', sans-serif" }}>
                        Press Space to start
                    </h1>
                    <div 
                        className="niokbutton" 
                        onClick={() => setShowMessage(false)}
                        style={{ cursor: 'pointer' }}
                    />
                </div>
            )}
            
            <div id="main-frame-error" className="interstitial-wrapper">
                <div id="main-content">
                    <div className="icon icon-offline"></div>
                </div>
                <div id="offline-resources">
                    <img 
                        id="offline-resources-1x" 
                        src="/t-rex/default_100_percent/100-offline-sprite.png" 
                        alt=""
                        onError={(e) => {
                            console.error('Failed to load 1x sprite:', e)
                            // Try fallback
                            const img = e.target as HTMLImageElement
                            img.src = '/t-rex/offline-sprite-1x.png'
                        }}
                    />
                    <img 
                        id="offline-resources-2x" 
                        src="/t-rex/default_200_percent/200-offline-sprite.png" 
                        alt=""
                        onError={(e) => {
                            console.error('Failed to load 2x sprite:', e)
                            // Try fallback
                            const img = e.target as HTMLImageElement
                            img.src = '/t-rex/offline-sprite-2x.png'
                        }}
                    />
                    <template id="audio-resources">
                        <audio 
                            id="offline-sound-press" 
                            src="data:audio/mpeg;base64,"
                        />
                        <audio 
                            id="offline-sound-hit" 
                            src="data:audio/mpeg;base64,"
                        />
                        <audio 
                            id="offline-sound-reached" 
                            src="data:audio/mpeg;base64,"
                        />
                    </template>
                </div>
            </div>
        </div>
    )
}

