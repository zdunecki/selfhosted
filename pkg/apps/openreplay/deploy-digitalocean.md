...
# Deploy to Digital Ocean - OpenReplay Documentation
...
[Section titled Create a droplet](#create-a-droplet)

1.  Create a new project (if you haven’t done so) then navigate to it and click on ‘Create’ droplet
2.  Choose the _Ubuntu Server 22.04 LTS_ image
3.  Go for the ‘Shared CPU’ and pick the `Regular Intel | 4 CPUs | 8GB` plan (x86 architecture). These are the minimum specs, otherwise OpenReplay backend services won’t simply start. They’re enough for a moderate volume, but if you’re expecting high traffic, you should scale from here.
...
[Section titled Deploy OpenReplay](#deploy-openreplay)
...
2.  Install OpenReplay by providing the domain on which it will be running (e.g. DOMAIN\_NAME=openreplay.mycompany.com):
...
[Section titled Configure TLS/SSL](#configure-tlsssl)
...
@sh
``` @sh
private_key_file.pem={opts.SSLPrivateKeyFile}
certificate.crt={opts.SSLCertificateCrt}
...
@sh
```@sh
80={opts.IngressHTTPPort}
443={opts.IngressHTTPsPort}
```
